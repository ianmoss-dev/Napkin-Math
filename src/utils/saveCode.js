...
export function generateSaveCode(userData) {
  const slim = {
    ...
    er: userData.emergencyReady,
    fm: userData.fullMatchStatus,
    cdt: userData.checklistDebtType,
    cdb: userData.checklistDebtBalance,
    cdr: userData.checklistDebtRate,
    cde: userData.checklistDebtRateEstimated,
    rsm: userData.retirementSavingsMonthly,
    smr: userData.savingMoreRetirement,
    sog: userData.savingOtherGoals,
...
export function decodeSaveCode(raw) {
  try {
    ...
    return {
      ...
      emergencyReady: slim.er ?? null,
      fullMatchStatus: slim.fm ?? null,
      checklistDebtType: slim.cdt ?? 'none',
      checklistDebtBalance: slim.cdb ?? 0,
      checklistDebtRate: slim.cdr ?? 0,
      checklistDebtRateEstimated: slim.cde ?? false,
      retirementSavingsMonthly: slim.rsm ?? 0,
      savingMoreRetirement: slim.smr ?? null,
      savingOtherGoals: slim.sog ?? null,
